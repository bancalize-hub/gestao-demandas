...
<template>
  <div style="flex:1;min-width:0;background:var(--c-bg-deep);display:flex;flex-direction:column;overflow-y:auto;">
    <div class="r-wrap" style="padding:16px clamp(12px,4vw,30px);border-bottom:1px solid var(--c-surface-1);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
      <!-- Nome de empresa longo + logo de até 160px estouravam os ~336px de um celular de 360px. -->
      <div class="r-wrap" style="display:flex;align-items:center;gap:11px;min-width:0;">
        <CrmBrandLogo :company="branding" fallback-name="Portal de solicitações" :height="34" />
        <div class="r-break" style="font-size:12px;color:var(--c-text-muted);">{{ branding?.name ? `${branding.name} · canal do cliente` : 'canal do cliente' }}</div>
      </div>
    ...
    <div style="flex:1;display:flex;align-items:flex-start;justify-content:center;padding:38px clamp(12px,4vw,30px) 60px;">
      <div style="width:640px;max-width:100%;">
        <div v-if="!crm.formSubmitted" class="r-xs-pad-sm" style="background:var(--c-bg);border:1px solid var(--c-surface-1);border-radius:18px;padding:30px;">
        ...
        <div v-else class="r-xs-pad-sm" style="background:var(--c-bg);border:1px solid var(--c-surface-1);border-radius:18px;padding:46px 40px;text-align:center;display:flex;flex-direction:column;align-items:center;gap:16px;">
          ...
          <div class="r-wrap" style="display:flex;gap:10px;margin-top:6px;justify-content:center;">
