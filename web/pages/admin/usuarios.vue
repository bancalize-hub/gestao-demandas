  ...
  <div style="flex:1;min-width:0;background:var(--c-bg-deep);display:flex;flex-direction:column;overflow-y:auto;">
    <div class="r-wrap" style="padding:16px clamp(12px,4vw,30px);border-bottom:1px solid var(--c-surface-1);display:flex;align-items:center;gap:11px;flex-shrink:0;">
      ...
      <div class="r-min0">
        ...
        <!-- em 360px o subtítulo ocupava 2 linhas e desalinhava o topo do ícone -->
        <div class="r-xs-hide" style="font-size:12px;color:var(--c-text-muted);">Acesso ao painel · somente administradores</div>
      </div>
    </div>

    <!-- o respiro lateral era 24px fixo: em 360px sumiam 48px de largura útil. Agora
         segue o mesmo clamp do cabeçalho desta página (12px no celular, 24px no desktop) -->
    <div style="flex:1;display:flex;align-items:flex-start;justify-content:center;padding:34px clamp(12px,4vw,24px) 60px;">
      <div style="width:760px;max-width:100%;display:flex;flex-direction:column;gap:24px;">
        <!-- novo usuário -->
        <!-- 24px de cartão + 24px do wrapper deixavam os campos com 264px em 360px -->
        <div class="r-xs-pad-sm" style="background:var(--c-bg);border:1px solid var(--c-surface-1);border-radius:16px;padding:24px;">
          ...
          <div style="margin-top:20px;display:flex;flex-direction:column;gap:16px;">
            ...
            <div class="r-col-1" style="display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:end;">
              ...
              <!-- concede acesso total: o alvo tinha 38px de altura. r-tap-h leva a LINHA
                   inteira (que já é o alvo, por ser <label>) a 44px, em vez de inchar o
                   quadradinho do checkbox para 44×44 -->
              <label class="r-tap-h" style="display:flex;align-items:center;gap:9px;font-size:13.5px;color:var(--c-text-secondary);cursor:pointer;padding:11px 0;"><input v-model="form.is_admin" type="checkbox" style="width:16px;height:16px;accent-color:var(--c-ai);cursor:pointer;flex-shrink:0;">Administrador</label>
            </div>
            ...
            <!-- ação principal do formulário: 43px de altura e encostada à esquerda;
                 no celular pequeno passa a ocupar a linha -->
            <button class="r-tap r-xs-full" :disabled="saving" :style="{ alignSelf: 'flex-start', background: 'var(--c-ai)', border: 'none', color: 'var(--c-on-accent)', fontFamily: 'inherit', fontSize: '14px', fontWeight: 700, padding: '12px 22px', borderRadius: '12px', cursor: saving ? 'default' : 'pointer', opacity: saving ? 0.7 : 1 }" @click="create">
          ...
          <div v-else style="display:flex;flex-direction:column;gap:10px;">
            <div v-for="u in users" :key="u.id" style="display:flex;align-items:center;gap:13px;background:var(--c-surface-2);border-radius:12px;padding:12px 15px;">
              ...
              <div style="flex:1;min-width:0;">
                <div class="r-break" style="font-weight:700;font-size:14px;">{{ u.name }}</div>
                <!-- em 360px sobram ~138px para nome+e-mail e a reticência engolia o
                     endereço ("contato.fin…"), justamente o dado que identifica a pessoa:
                     no celular é melhor o e-mail quebrar em duas linhas -->
                <div class="r-sm-unclamp r-break" style="font-size:12.5px;color:var(--c-text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ u.email }}</div>
              </div>
