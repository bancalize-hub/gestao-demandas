  ...
  <div style="display:flex;height:100dvh;background:var(--c-bg-deepest);color:var(--c-text);font-family:'JetBrains Mono',ui-monospace,monospace;position:relative;overflow:hidden;">
    ...
    >
      ...
      <div style="flex:1;overflow-y:auto;padding:4px 8px;min-height:0;">
        <div v-for="s in sessions" :key="s.id" :style="{ display:'flex',alignItems:'center',gap:'6px',padding:'9px 10px',borderRadius:'9px',cursor:'pointer',marginBottom:'3px',background: s.id===activeId ? 'var(--c-surface-0)' : 'transparent' }" @click="openSession(s.id)">
          <span style="flex:1;font-size:12.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--c-text);">{{ s.title }}</span>
          <button title="Apagar" style="background:none;border:none;color:var(--c-text-faint);cursor:pointer;font-size:14px;" @click.stop="delSession(s.id)">×</button>
        </div>
      </div>
      ...
      <template v-else>
        <div ref="scroller" style="flex:1;overflow-y:auto;min-height:0;padding:18px clamp(12px, 4vw, 40px);display:flex;flex-direction:column;gap:14px;">
          <template v-for="m in messages" :key="m.id">
            ...
            <div v-else style="align-self:stretch;background:var(--c-bg-deep);border:1px solid var(--c-surface-1);border-radius:12px;padding:14px 16px;font-size:12.5px;line-height:1.55;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;color:var(--c-text);">{{ m.content }}</div>
          </template>
          <!-- saída ao vivo do job em execução -->
          <div v-if="running" style="align-self:stretch;background:var(--c-bg-deep);border:1px solid var(--accent-deep);border-radius:12px;padding:14px 16px;font-size:12.5px;line-height:1.55;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;color:var(--c-text);">{{ liveOutput || '⏳ iniciando…' }}<span style="color:var(--accent);animation:blink 1s infinite;">▋</span></div>
        </div>
